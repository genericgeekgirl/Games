!----------------------------------------------------------------------------
!
!         Hugo Object Library v2.5.03.5 by Kent Tessman (c) 1995-2001
...
direction spectre_n_obj "north"
{
	in direction
	!nouns "n", "north"
	dir_to spectre_n_to
	dir_from spectre_s_obj
}

direction spectre_s_obj "south"
{
	in direction
	!nouns "s", "south"
	dir_to spectre_s_to
	dir_from spectre_n_obj
}
	
direction spectre_w_obj "west"
{
	in direction

	dir_to spectre_w_to
	dir_from spectre_e_obj
}

direction spectre_e_obj "east"
{
	in direction

	dir_to spectre_e_to
	dir_from spectre_w_obj
}
...
routine DoMoveinVehicle
{
	...
	! Match the verb
	if not InList(v, vehicle_verbs, word[1])
...
! less than specified (or any at all, for that matter).
			...
			{
				if self.plural_of #i is workflag
				{
...
! their respective holding classes:
	...
	pluralobj_heldmode = 0  ! i.e. not explicitly held or not
...
! VerbHeldmode routine checks to see if the verb necessarily implies a
...
! phrasing instead--change the plural object word to represent <n>.
	...
	if plural_type = single_noun and pobj.type ~= identical_class and any
	{
		SetObjWord(w, n)
...
! the component members is available.  The same goes for identical_classes.

	elseif n and (plural_type = noun or pobj.type = identical_class)
	{
...
! VerbHeldmode sets the pluralobj_heldmode global to the appropriate value
...
routine VerbHeldmode(a)
{
	if a = "get", "take", "pick", "remove":
		pluralobj_heldmode = -1         ! explicitly notheld

	elseif a = "drop", "put", "leave", "place", "throw",
		"give", "offer":
		pluralobj_heldmode = 1          ! explicitly held
...
routine DoAttachObject
{
	...
	! Match the verb
	if not InList(object, attach_verbs, word[1])
	...
	if not InList(object, attachable_to, xobject)
	{
	...
	{
		! "Except that it is already (attached) to..."
...
routine DoDetachObject
{
	...
	! Match the verb
	if not InList(object, detach_verbs, word[1])
...
routine ObjectAttach(att, obj)
{
	...
	if not InList(att, attachable_to, obj):  return false
...
! Check first the children of the object in question:

	check = Check_AttachableChildren(obj, newloc)
	if check:  jump SomethingisAttached

! Failing finding any attachments there, use the roundabout method of
! checking the location:

	check = Check_Attachedto(obj, oldloc, newloc)
	if check:  jump SomethingisAttached
...
! Check_Attachable checks all the attachments of <att> for mobility, where
! <att> is an attachable.  Returns the number of the immobile object, or
! false if none.

routine Check_Attachable(att, newloc)
{
	local a, b, i
	...
	for (i=1; i<=att.#attached_to; i++)
	{
...
#ifset USE_VEHICLES
		if a.type = vehicle and not InList(newloc, vehicle_path, a)
		{
			! "You can't pull..."
			OMessage(attachable, 20, a)
			return -1
		}
#endif
		if a and (a is not mobile and not Contains(a, att))
		{
			if a.attach_immobile
				return -1
			return att
		}
		...
		if a.#attachable_to
		{
			b = Check_Attachable(a, newloc)
			if b:  return b
		}

		if a and child(a) and a ~= parent(att)
		{
			b = Check_AttachableChildren(a, newloc)
			if b:  return b
		}
	}
}
...
! Check_AttachableChildren checks to see what (if anything) in <obj> is
! an attachable.

routine Check_AttachableChildren(obj, newloc)
{
	local b, i
	...
	for i in obj
	{
		if i.#attached_to
		{
			b = Check_Attachable(i, newloc)
			if b:  return b
		}
		elseif child(i)
		{
			b = Check_AttachableChildren(i, newloc)
			if b:  return b
		}
	}
}
...
! Check_Attachedto checks attachables in <obj>'s location.  Returns the
! number of the first attachable keeping <obj> from moving; otherwise
! returns false.

routine Check_Attachedto(obj, oldloc, newloc)
{
	local i, j, k
	...
	for i in oldloc
	{
		for (j=1; j<=i.#attached_to; j++)
		{
			...
			if k = obj or Contains(obj, k):
			{
#ifset USE_VEHICLES
				if k.type = vehicle and
					not InList(newloc, vehicle_path, k):
				{
					! "You can't pull..."
					OMessage(attachable, 20, k)
					return -1
				}
#endif

				if k is not mobile and
					k~=obj and not Contains(obj, k)
				{
					if k.attach_immobile
						return -1
					return i
				}
			}
		...
		if child(i)
		{
			k = Check_Attachedto(obj, child(i))
			if k
				return k
		}
	}
...
! First, move all attachables in the specified object:
	...
	first = Move_Attachables(obj, oldloc, newloc)

! Then, move anything else in the old location that might somehow be
! attached to it:

	second = Move_Attachedto(obj, oldloc, newloc)
...
! Move_Attachables moves all objects attached by an attachable item in <obj>.

routine Move_Attachables(obj, oldloc, newloc)
{
	local i, k, first

	for i in obj
	{
		k = Move_AttachedObjects(i, obj, oldloc, newloc)
...
! Move_AttachedObjects is called to move everything attached to the
! attachable <att> in <oldloc> to <newloc>.

routine Move_AttachedObjects(att, obj, oldloc, newloc)
{
	local i, j, first
		...
		for (i=1; i<=att.#attached_to; i++)
		{
			...
			if j
			{
				if Contains(oldloc, j) and j not in obj
				{
					...
					move j to newloc
					j is moved
					Move_Attachables(j, oldloc, newloc)
					Move_Attachedto(j, oldloc, newloc)
				}
			}
		}
	}
...
! Move_Attachedto scans <oldloc> for anything attached to <obj>, moving
...
routine Move_Attachedto(obj, oldloc, newloc)
{
	local i, j, k, first
	...
	for i in oldloc
	{
		for (j=1; j<=i.#attached_to; j++)
		{
				...
				Contains(obj, i.attached_to #j):
			{
				...
				k = Move_AttachedObjects(i, obj, oldloc, newloc)
				if not first:  first = k
				k = Move_Attachables(i, obj, oldloc, newloc)
				if not first:  first = k
			}
		}
		...
		if child(i)
		{
			k = Move_Attachedto(obj, child(i), newloc)
			if not first:  first = k
		}
	}
	return first
...
routine OMessage(obj, num, a, b)
{
	...
	case door
	{
		...
		case 5
		{
			...
			print ", closing it behind "; actor.pronoun #2;
			if actor is plural
				print "selves."
			else
				print "self."
	...
	case attachable
	{
		...
		case 6:  print CThe(player); " can't "; word[1]; " "; \
			The(object); "."
		case 7:  print "You might want to try \""; a; "\" with "; \
			The(object); " instead of "; "\""; word[1]; "\"."
		case 8:
		{
			print "\B[Be a little more specific
				about what you'd like ";
			if player_person ~= 2:  print The(player, true); " ";
			print "to "; object.attach_verb; " "; \
			The(object); " "; object.attach_prep; ".]\b"
		}
		case 9:  print "(taking "; The(object); " first)"
		case 10:  print CThe(player); " can't "; word[1]; " "; \
		...
		case 16:
		{
			print "\B[Be a little more specific
				about what you'd like ";
			if player_person ~= 2:  print The(player, true); " ";
			print "to "; word[1]; " "; The(object); " "; \
				object.detach_prep; "."
		}
		case 17:  print CThe(player); " "; object.detach_verb; \
			MatchSubject(player); " "; The(object); " "; \
			object.detach_prep; " "; The(xobject); ".]\b"
		case 18
		{
