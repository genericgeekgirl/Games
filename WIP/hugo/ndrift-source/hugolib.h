!----------------------------------------------------------------------------
!
!           Hugo Library v3.0.05.1 by Kent Tessman (c) 1995-2002
...
constant BANNER "Hugo v3.0 / Library 30051"
constant HUGO_VERSION   "v3.0"
...
! Robb -- placed this here for Necrotic Drift in order to move the Spectre 
property spectre_n_to
property spectre_s_to
property spectre_w_to
property spectre_e_to
...
global player_person = 1                ! 1=first, 2=second, 3=third
...
global parser_status			! PARSER_STATUS mask
global best_parse_rank                  ! for matching similarly named objects
global parse_rank_tests			! counter
...
! PARSER STATUS
!
! For parser_status.  Mask must be reset in Parse, as well as after
! Perform and ParseError.
!----------------------------------------------------------------------------
enumerate step *2
{
	PARSER_RESET,		! 0, for reset
	PARSER_ACTIVE = 1,
	PRONOUNS_SET,		! if "it", etc. is used
...
!
! Default responses (in PrintEndGame):  1 = win, 2 = die, 3 = no response
...
! Robb -- Changed this for Necrotic Drift 
...
routine PrintScore(end_of_game)
{
	...
	{
	print "\B[Your score is ";
	print number score; " from a trivial "; number MAX_SCORE; ".]\b"
	}
...
! something" so that the engine automatically reroutes them to SpeakTo
	...
	elseif word[1] = "tell", "order", "instruct", "ask", "command"
	{
		...
		if not reparse
		{
			if words = 3 
			{
				if ObjWord(2) and not ObjWord(3)
				{
					word[1] = word[2]
					word[2] = "~and"
					reparse = true
	...
	for (count=2; count<=words and word[count]~=""; count++)
	{
		select word[count]
			...
			case "it", "itself", "them", "themselves", \
				"him", "himself", "her", "herself"
			{
				select word[count]
					case "it", "itself":       a = it_obj
					case "them", "themselves": a = them_obj
					case "him", "himself":     a = him_obj
					case "her", "herself":     a = her_obj
				...
				if word[count] ~= word[count+1]
				{
					...
					! replace "her" where it's the second-to-last
					! word:
					if a ~= her_obj or
...
routine ParseError(errornumber, obj)
{
	...
	select errornumber
		...
		case 1
			! Robb -- changed 
			print "\B[ The word \""; parse$; "\" is not recognized by the game. ]\b"
			!\
			print CThe(player); \
				! " can't use the word \""; \
		...
		case 5
		{
			if word[1] = "give", "offer", "hand", "show"
			  print "Not sure what you're referring to--try \""; \
				word[1]; " (something) to (someone)\"."
			else
			{
			  print CThe(player); \
		...
		case 6
			...
			print "\B[ That sentence is not understood by the game. ]\b"
		...
		{
			if obj is living
			print "I can't see that person right now."
			else:
			print "I can't see that right now."
		...
		case 11
		{
			if not ObjectisKnown(obj) and
				not (parser_status & FINDOBJECT_KNOWN)
			{
				ParseError(10, obj)
			}
...
routine Perform(action, obj, xobj, queue)
{
	local r
	...
	if debug_flags & D_PARSE
	{
		print "\B[Perform("; number action; ", "; object.name;
...
! AfterRoutines
!
! Runs all relevant routines that might follow a verbroutine, returning
! true if any of them returns true.  Note that object.after and xobject.after
! are not called here, nor is anything that should be explicitly called by
! a verbroutine depending on its (degree of) failure or success.
!----------------------------------------------------------------------------
...
{
#ifclear NO_VERBS
	if verbroutine >= &DoBrief and verbroutine <= &DoUndo
		return false
	else
		return obj
#endif
...
routine FindObject(obj, objloc, recurse)
{
	...
	if objloc = 0
	{
		...
		if verbroutine = &DoGet and not recurse
		{
			if obj in player
			{
				...
				{
					print "[FindObject("; The(obj); ", "; The(objloc); "):  \
		...
		if not recurse
		{
			if parse_rank_tests++ = 0
			{
				best_parse_rank = obj.parse_rank
#ifset DEBUG
				if debug_flags & D_FINDOBJECT and not recurse
				{
					print "[FindObject("; The(obj); ", "; The(objloc); "):  true]"
				}
#endif
				return true
			}
			...
			if obj.parse_rank < best_parse_rank
			{
				...
				{
					print "[FindObject("; The(obj); ", "; The(objloc); "):  \ 
						false (obj.parse_rank < best_parse_rank)]"
				}
#endif
				return false
			}
	...
	if parser_status & PARSER_ACTIVE
	{
		if ObjectisKnown(obj)
			parser_status |= FINDOBJECT_KNOWN
	}
	...
	if obj.#in_scope
	{
		for (a=1; a<=obj.#in_scope; a++)
		{
			...
			if obj.in_scope #a and
				FindObject(obj.in_scope #a, objloc, true):
			{
				obj is known
				jump FindObjectFound
			}
	...
	if debug_flags & D_FINDOBJECT and not recurse
	{
		if obj.type ~= fuse and obj.type ~= daemon
			print "[FindObject("; The(obj); ", "; The(objloc); "):  false]"
	}
#endif
	return false
...
:FindObjectFound
	if not recurse
	{
		if parse_rank_tests++ = 0
			best_parse_rank = obj.parse_rank
		elseif obj.parse_rank > best_parse_rank
			best_parse_rank = obj.parse_rank
	}
#ifset DEBUG
	if debug_flags & D_FINDOBJECT and not recurse
...
routine AssignPronoun(obj)
{
	...
	if obj >= it_object and obj <= them_object
		return
#endif
	if parser_status & PRONOUNS_SET:  return
...
routine DescribePlace(place, long)
{
	...
	if not place.list_contents
	{
		...
		! Added by Robb -- I think it would be better if I called my (tested) Determine_Followers routine
		! here rather than do the code that appears right after

		! What the following was doing was placing them in a room too soon....

!		Determine_Followers()
!\

		! Added by Robb -- add characters that are following 
		for (x_char = 1; x_char <= NUMCHARACTERS; x_char++)
		{
			if Characters[x_char] is following
			{
				!"Did this part."
				move Characters[x_char] to place
			}
		}
\!
...
!\
		Robb -- I don't want the game doing the below. 
...
!		Robb - Changed this so that living objects don't list their contents 
!		(obj ~= player or FORMAT & INVENTORY_F) and obj is not quiet:

		(obj ~= player or FORMAT & INVENTORY_F) and (obj is not living or obj = Player) and (obj is not quiet_bcast) and obj is not quiet:
	{
...
{
	local i, k, obj, count, pluralcount
	...
	if not (FORMAT & LIST_F)
	{
		...
		! Print the post-period spacing, but try to figure out if
		! it's necessary, i.e., if the last object has been listed,
		! and the period was printed in the very last column
		!
		if count and k and i = 0
		{
			if list_nest > 1 and
				display.cursor_column < display.linelength:
				print AFTER_PERIOD;
		}
	}
...
attribute active alias switchedon
...
!----------------------------------------------------------------------------
! Include verb library unless otherwise specified
